...
// Add this after imports, before the templates constant
...
export default function NewResumePage() {
  ...
  const extractTextFromFile = async (file: File): Promise<string> => {
    ...
    if (fileType.includes('word')) {
      // You'll need to install and import mammoth
  ...
  // Function to parse job details from text
  const parseJobDetails = (text: string) => {
    const details = {
      title: '',
      company: '',
      location: '',
    ...
    const patterns = {
      ...
      company: [
        ...
        /organization[:\s]+([^\n]+)/i,
        /employer[:\s]+([^\n]+)/i,
        /(?:at|with|for)\s+([A-Z][A-Za-z0-9\s&]+?)(?:\sin|\s-|\s–|\.|$)/
      ],
      location: [
        /location[:\s]+([^\n]+)/i,
        /place[:\s]+([^\n]+)/i,
        /city[:\s]+([^\n]+)/i,
        /(?:in|at)\s+([A-Z][A-Za-z\s,]+(?:,[A-Z]{2})?)/,
        /([A-Z][A-Za-z\s]+(?:,[A-Z]{2}))(?:\s|$)/,
        /remote/i
      ]
    };
  
    // Try to find matches for each field with logging
    for (const [field, regexList] of Object.entries(patterns)) {
      for (const regex of regexList) {
        const match = text.match(regex);
        console.log(`Trying ${field} with regex ${regex}:`, match);
        if (match && match[1]) {
          details[field as keyof typeof details] = match[1].trim();
    ...
    if (!details.title && text.length > 0) {
      // Try to extract first line as title if nothing else found
  ...
  // Update the handleJobFileUpload function
  const handleJobFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setJobFileError('File size must be less than 5MB');
      return;
    }
    ...
    if (!validTypes.includes(fileExt)) {
      setJobFileError(`Invalid file type. Allowed types: ${validTypes.join(', ')}`);
      return;
    }
    ...
    try {
      ...
      // Update state with extracted information in the requested format
      setJobDetails(prev => ({
        ...prev,
        title: details.title || "Position Not Found",
        company: details.company || "Company Not Found",
      ...
      setJobFileError('Failed to parse job description from file');
    }
  };
              ...
              <TabsContent value="file">
                <div className="text-center py-10 border-2 border-dashed border-gray-200 rounded-lg">
                  ...
                  <p className="text-gray-500 mb-4">
                    Drag and drop a file or click to browse. Supported formats: PDF, DOC, DOCX, TXT
                  </p>
                  <div className="space-y-4">
                    ...
                    {jobFile && (
                      <div className="text-sm font-medium text-green-600">
                        {jobFile.name} uploaded successfully
                      </div>
                    )}
                    {jobFileError && (
                ...
                {(jobDetails.title || jobDetails.company || jobDetails.description) && (
